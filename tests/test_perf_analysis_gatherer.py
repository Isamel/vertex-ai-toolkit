"""Integration tests for the Perf Analysis skill gatherer agent.
...
class TestPerfAnalysisGathererAgent:
    """Gatherer agent wiring tests for PerfAnalysisSkill."""
    ...
    def test_gatherer_is_first_agent(self) -> None:
        ...
        assert agents[0]["name"] == "perf_gatherer"
    ...
    def test_gatherer_prompt_starts_with_anti_injection(self) -> None:
        from vaig.skills.perf_analysis.prompts import (
            ANTI_INJECTION_RULE,
            PERF_ANALYSIS_GATHERER_PROMPT,
        )

        assert PERF_ANALYSIS_GATHERER_PROMPT.startswith(ANTI_INJECTION_RULE)
...
class TestPerfAnalysisSkillLiveToolsMetadata:
