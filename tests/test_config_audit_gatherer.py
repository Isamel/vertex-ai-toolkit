"""Integration tests for the Config Audit skill gatherer agent.
...
class TestConfigAuditGathererAgent:
    """Gatherer agent wiring tests for ConfigAuditSkill."""
    ...
    def test_gatherer_is_first_agent(self) -> None:
        ...
        assert agents[0]["name"] == "config_gatherer"
    ...
    def test_gatherer_prompt_starts_with_anti_injection(self) -> None:
        from vaig.skills.config_audit.prompts import (
            ANTI_INJECTION_RULE,
            CONFIG_AUDIT_GATHERER_PROMPT,
        )

        assert CONFIG_AUDIT_GATHERER_PROMPT.startswith(ANTI_INJECTION_RULE)
...
class TestConfigAuditSkillLiveToolsMetadata:
