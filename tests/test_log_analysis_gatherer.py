"""Integration tests for the Log Analysis skill gatherer agent.
...
class TestLogAnalysisGathererAgent:
    """Gatherer agent wiring tests for LogAnalysisSkill."""
    ...
    def test_gatherer_is_first_agent(self) -> None:
        ...
        assert agents[0]["name"] == "log_analysis_gatherer"
    ...
    def test_gatherer_prompt_starts_with_anti_injection(self) -> None:
        from vaig.skills.log_analysis.prompts import (
            ANTI_INJECTION_RULE,
            LOG_ANALYSIS_GATHERER_PROMPT,
        )

        assert LOG_ANALYSIS_GATHERER_PROMPT.startswith(ANTI_INJECTION_RULE)
...
class TestLogAnalysisSkillLiveToolsMetadata:
