...
class TestThrottleCodePath:
    ...
    @patch("vaig.agents.mixins.time.sleep")
    @patch("vaig.agents.mixins.get_settings")
    def test_throttle_sleeps_on_second_iteration(
        self,
        mock_get_settings: MagicMock,
        mock_sleep: MagicMock,
    ) -> None:
        """time.sleep IS called with the configured delay on the second iteration."""
        from vaig.agents.mixins import ToolLoopMixin
        ...
        # First call: model returns a function call → triggers second iteration
        mock_fc = MagicMock()
        mock_fc.name = "test_tool"
        mock_fc.args = {}

        first_result = MagicMock()
        first_result.text = None
        first_result.function_calls = [mock_fc]
        first_result.tool_calls = []
        first_result.usage = {"prompt_tokens": 10, "completion_tokens": 10, "total_tokens": 20}
        first_result.finish_reason = "STOP"

        # Second call: model returns text → loop ends
        second_result = MagicMock()
        second_result.text = "Done"
        second_result.function_calls = []
        second_result.tool_calls = []
        second_result.usage = {"prompt_tokens": 10, "completion_tokens": 10, "total_tokens": 20}
        second_result.finish_reason = "STOP"

        mock_client = MagicMock()
        mock_client.generate_with_tools.side_effect = [first_result, second_result]

        mock_exec_result = MagicMock()
        mock_exec_result.to_part.return_value = MagicMock()
        mock_exec_result.execution_error = None
        mock_exec_result.execution_result = "ok"

        mock_registry = MagicMock()
        mock_registry.to_function_declarations.return_value = []
        mock_registry.execute.return_value = mock_exec_result
        ...
        # Second iteration triggers throttle sleep
