...
def discover_workloads(
    *,
    ...
    # ── Cache check ───────────────────────────────────────────
    cache_key = _cache_key_discovery("workloads", namespace, str(include_jobs))
    ...
    for rtype in resource_types:
        try:
            res = _list_resource(core_v1, apps_v1, custom_api, rtype, namespace, api_client=api_client)
            ...
            for item in items:
                ...
                age = _age(meta.creation_timestamp)
...
def discover_service_mesh(
    *,
    ...
    # ── Cache check ───────────────────────────────────────────
    cache_key = _cache_key_discovery("mesh", namespace)
    if not force_refresh:
        cached = _get_cached(cache_key)
        if cached is not None:
            return ToolResult(output=cached, error=False)
...
def discover_network_topology(
    *,
    ...
    # ── Cache check ───────────────────────────────────────────
    cache_key = _cache_key_discovery("network", namespace)
    if not force_refresh:
        cached = _get_cached(cache_key)
        if cached is not None:
            return ToolResult(output=cached, error=False)
    ...
    return [
        ...
        ToolDef(
            ...
            execute=lambda namespace="", include_jobs=False, force_refresh=False,
                    _cfg=gke_config: discover_workloads(
        ...
        ToolDef(
            ...
            execute=lambda namespace="", force_refresh=False,
                    _cfg=gke_config: discover_service_mesh(
        ...
        ToolDef(
            ...
            execute=lambda namespace="", force_refresh=False,
                    _cfg=gke_config: discover_network_topology(
                gke_config=_cfg, namespace=namespace, force_refresh=force_refresh,
            ),
        ),
    ]
